...
export default function YesNoNotGiven({ question = {}, value = '', onChange = () => {}, disabled = false }) {
            ...
            <div className="grid gap-2">
                {options.map((opt) => (
                    <label key={opt} className={`flex items-center gap-3 p-3 rounded-xl border ${value === opt ? 'border-blue-500 bg-blue-50' : 'border-slate-100 bg-white'}`}>
                        <input type="radio" name={`ynng-${question.id}`} value={opt} checked={value === opt} onChange={() => !disabled && onChange(opt)} disabled={disabled} />
