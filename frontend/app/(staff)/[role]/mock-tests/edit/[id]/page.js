...
import { ChevronLeft, Clock, FilePenLine, Loader2, Save, Settings2 } from "lucide-react";
import {
    useGetMockTestQuery,
...
export default function EditMockTestPage() {
    ...
    const { data: test, isLoading: fetching } = useGetMockTestQuery(id);
    const [updateTest, { isLoading: saving }] = useUpdateMockTestMutation();
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            setError(getRequestMessage(requestError, "Unable to update mock test."));
        }
    }
    ...
    if (fetching) {
        return (
            <div className="flex h-[400px] flex-col items-center justify-center gap-4 text-slate-500">
                ...
                <p className="font-medium">Retrieving mock test...</p>
            </div>
        );
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <section className="rounded-[26px] bg-[linear-gradient(150deg,_#111827,_#1f2937)] p-8 text-white shadow-xl">
                    <div className="flex items-center gap-3 opacity-80">
                        ...
                        <p className="text-xs uppercase tracking-[0.28em]">Mock Assessment Update ID: {id}</p>
                    </div>
                    <h2 className="mt-4 text-3xl font-bold">Edit Mock Test</h2>
                    <p className="mt-3 text-slate-400 max-w-2xl leading-relaxed">
                        Update the mock exam identity, timing, category, and access settings.
                    </p>
                </section>
                ...
                <div className="rounded-[26px] border border-slate-200 bg-white p-8 shadow-sm space-y-8">
                    ...
                    <button type="submit" disabled={saving} className="flex items-center gap-2 rounded-2xl bg-slate-950 px-8 py-3.5 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5 disabled:opacity-60">
                        ...
                        {saving ? "Updating..." : "Update Mock Test"}
                    </button>
                </div>
            </form>
        </div>
