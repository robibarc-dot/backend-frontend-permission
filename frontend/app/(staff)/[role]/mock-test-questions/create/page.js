...
import { useCreateMockTestQuestionMutation } from "@/redux/features/mock/backend/mockTestQuestionApi";
...
export default function CreateMockTestQuestionPage() {
    const router = useRouter();
    const { role } = useParams();
    const resolvedRole = role?.toLowerCase();
    const [createAssignment, { isLoading: saving }] = useCreateMockTestQuestionMutation();
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            setTimeout(() => router.push(`/${resolvedRole}/mock-test-questions`), 1500);
        } catch (requestError) {
    ...
    return (
        <div className="mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <Link
                    href={`/${resolvedRole}/mock-test-questions`}
