...
export default function EditQuestionTypePage() {
    ...
    const { data: questionType, isLoading: fetchingType, isError: isErrorFetchingType } = useGetQuestionTypeQuery(id);
    const [updateQuestionType, { isLoading: isUpdating }] = useUpdateQuestionTypeMutation();
    ...
    useEffect(() => {
        if (questionType) {
            setForm({
                ...
                module_id: questionType.module_id || "",
                upload_hints: questionType.upload_hints || "",
            });
        }
    }, [questionType]);
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            setError(getRequestMessage(requestError, "Unable to update question type."));
        }
    }

    if (fetchingType || loadingModules) {
        return (
    ...
    if (isErrorFetchingType) {
        return <div className="p-6 text-red-500 font-medium">Error loading question type.</div>;
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <section className="rounded-[26px] bg-[linear-gradient(150deg,_#111827,_#1f2937)] p-8 text-white shadow-xl">
                    <div className="flex items-center gap-3 opacity-80">
                        ...
                        <p className="text-xs uppercase tracking-[0.28em]">Question Type ID: {id}</p>
                    </div>
                    <h2 className="mt-4 text-3xl font-bold">Edit Question Type</h2>
                    <p className="mt-3 text-slate-400 max-w-2xl leading-relaxed">
                        Modify the details of this question type.
                    </p>
                </section>
                ...
                <div className="rounded-[26px] border border-slate-200 bg-white p-8 shadow-sm space-y-8">
                    ...
                    <button type="submit" disabled={isUpdating} className="flex items-center gap-2 rounded-2xl bg-slate-950 px-8 py-3.5 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5 disabled:opacity-60">
                        {isUpdating ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}
                        {isUpdating ? "Updating..." : "Update Question Type"}
                    </button>
                </div>
            </form>
        </div>
