...
} from "@/redux/features/practice-test/backend/practiceTestQuestionApi";
import {
    getPrimaryRole,
    getRoleHomePath,
} from "../../../../lib/auth";
...
export default function PracticeTestQuestionsPage() {
    ...
    const { data: assignments = [], isLoading, isError } = useGetPracticeTestQuestionsQuery();
    const [deleteAssignment, { isLoading: isDeleting }] = useDeletePracticeTestQuestionMutation();
    ...
    const assignmentItems = assignments.map((assignment) => ({
        ...assignment,
        name: `Practice test question #${assignment.id}`,
        title: assignment.practice_test?.title || `Mock Test #${assignment.practice_test_id}`,
        title: assignment.module?.title || `Module #${assignment.module_id}`,
        question_label: getQuestionLabel(assignment),
    }));

    const filterConfigs = [
        { key: "title", label: "Mock Test", type: "text", placeholder: "Search mock test..." },
        { key: "question_label", label: "Question", type: "text", placeholder: "Search question..." },
        { key: "title", label: "Module", type: "text", placeholder: "Search module..." },
    ];
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (item) => (
                <Link
                    href={`/${resolvedRole}/practice-test-questions/edit/${item.id}`}
    ...
    return (
        <div className="p-6 max-w-[1600px] mx-auto space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <Link
                    href={`/${resolvedRole}/practice-test-questions/create`}
