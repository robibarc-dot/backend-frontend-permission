...
import { useCreatePracticeTestQuestionMutation } from "@/redux/features/practice-test/backend/practiceTestQuestionApi";
...
function toPayload(form) {
    return {
        practice_test_id: Number(form.practice_test_id),
...
export default function CreatePracticeTestQuestionPage() {
    const router = useRouter();
    const { role } = useParams();
    const resolvedRole = role?.toLowerCase();
    const [createAssignment, { isLoading: saving }] = useCreatePracticeTestQuestionMutation();
    ...
    async function handleSubmit(event) {
        ...
        try {
            await createAssignment(toPayload(form)).unwrap();
            setSuccess("Question assigned successfully.");
            setTimeout(() => router.push(`/${resolvedRole}/practice-test-questions`), 1500);
        } catch (requestError) {
            setError(getRequestMessage(requestError, "Unable to assign question."));
        }
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <section className="rounded-[26px] bg-[linear-gradient(150deg,_#111827,_#1f2937)] p-8 text-white shadow-xl">
                    <div className="flex items-center gap-3 opacity-80">
                        ...
                        <p className="text-xs uppercase tracking-[0.28em]">Mock Question Assignment</p>
                    </div>
                    <h2 className="mt-4 text-3xl font-bold">Assign Question</h2>
                    <p className="mt-3 text-slate-400 max-w-2xl leading-relaxed">
                        Connect a question to a mock test, choose its module, and set its display order.
                    </p>
                </section>
                ...
                <div className="rounded-[26px] border border-slate-200 bg-white p-8 shadow-sm space-y-8">
                    <div className="grid gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <input
                                ...
                                min="1"
                                required
                                value={form.practice_test_id}
                                onChange={(event) => updateField("practice_test_id", event.target.value)}
                    ...
                    <div className="rounded-2xl bg-slate-50 p-6 border border-slate-100">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <span className="px-2.5 py-1 bg-white border border-slate-200 rounded-lg text-[11px] font-medium text-slate-600">Mock Test: {form.practice_test_id || "Not set"}</span>
