...
import { UserPlus, ChevronLeft, Save, Loader2, ShieldCheck } from "lucide-react";
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            if (validationErrors) {
                ...
                setError(requestError?.data?.message || "Unable to create user.");
            }
        }
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <section className="rounded-[26px] bg-[linear-gradient(150deg,_#111827,_#1f2937)] p-8 text-white shadow-xl">
                    <div className="flex items-center gap-3 opacity-80">
                        ...
                        <p className="text-xs uppercase tracking-[0.28em]">Identity Management</p>
                    </div>
                    <h2 className="mt-4 text-3xl font-bold">Create New User</h2>
                    <p className="mt-3 text-slate-400 max-w-2xl leading-relaxed">
                        Set up a new account by providing basic details and assigning appropriate roles and individual permissions.
                    </p>
                </section>
                ...
                <div className="rounded-[26px] border border-slate-200 bg-white p-8 shadow-sm space-y-8">
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-slate-700 mb-6">Assign Roles</h3>
                        {metaLoading ? (
                    ...
                    <div className="rounded-2xl bg-slate-50 p-6 border border-slate-100">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            )) : <p className="text-xs text-slate-400 italic">No permissions inherited from roles yet.</p>}
                        </div>
                    </div>

                    <div>
                        <h3 className="text-sm font-semibold text-slate-700 mb-6">Direct Permissions Override</h3>
                        {metaLoading ? (
                             <div className="flex items-center gap-3 py-10 text-slate-400 justify-center"><Loader2 className="animate-spin" size={20} /></div>
                        ) : (
                            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                    ...
                                    return (
                                        <label key={perm.name} className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-4 transition-all duration-200 ${isChecked ? "border-blue-200 bg-blue-50/50" : "border-slate-100 bg-white"}`}>
                                            ...
                                            <div>
                                                ...
                                                {isInherited && <span className="text-[10px] text-emerald-600 font-semibold uppercase">Already Inherited</span>}
                                            </div>
                                        </label>
                                    );
                                })}
                    ...
                    <button type="submit" disabled={saving} className="flex items-center gap-2 rounded-2xl bg-slate-950 px-8 py-3.5 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5 disabled:opacity-60">{saving ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}{saving ? "Creating..." : "Save User"}</button>
                </div>
            </form>
        </div>
    );
